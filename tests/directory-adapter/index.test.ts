...
vi.mock("@/lib/adapters/directory-adapter/variables", async (importOriginal) => {
  const original = await importOriginal<typeof import("@/lib/adapters/directory-adapter/variables")>();
  return {
    ...original,
...
type DirectoryTestContext = {
  ...
  environments: Array<{ resolvedVariables: Partial<variables.ConfigResolutionVariables>; expected: unknown; }>;
}
...
describe.each<DirectoryTestContext>([
  ...
  beforeAll(async function() {
    ...
    await Promise.all(files.map(async ({ fileName, content }) => await writeFile(fileName, content)));
  })

  afterAll(async function() {
    await Promise.all(directories.map(async directory => await rm(directory, { recursive: true }) ))
  })
})
